...
//------------------------------CONSTANTES PARA EL BULBO SECO
const float Rcs = 99600; //valor de la resistencia
const float As = 0.7151582349e-3 ; // 1.116098329e-3 / Valores para NTC 10K
const float Bs = 2.145500343e-4; // 2.390698471e-4    /
const float Cs = 1.052559858e-7; // 0.6569096685e-7    /
...
//------------------------------CONSTANTES PARA EL BULBO HUMEDO

const float Rch = 99600; //valor de la resistencia
const float Ah = 0.7350098811e-3 ; // 1.116098329e-3 / Valores para NTC 10K
const float Bh = 2.119747688e-4; // 2.390698471e-4    /
const float Ch = 1.113550816e-7; // 0.6569096685e-7    /
...
// Configuració para que inicie promedio  
...
//------------------------------bulbo seco
  float TemperaturaSeco ()
                                 {                                     
  totalBS = totalBS - readingsBS[readIndexBS];
  readingsBS[readIndexBS] = analogRead(InputSeco);
  totalBS = totalBS + readingsBS[readIndexBS];
  readIndexBS = readIndexBS + 1;
  
  if (readIndexBS >= numReadingsBS)
    readIndexBS = 0;
    
  averageBS = totalBS / numReadingsBS;
  
  // Calculo Temperatura Bulbo Seco-Promedio 
                                 
  float VRs = averageBS;                                 
  Vs =( VRs )*( Vcc/1023.0);   
  Rseco = (Rcs * Vs ) / (Vcc - Vs);
  logRs=log(Rseco);
  R_ths = 1.0 / (As + Bs * logRs + Cs * logRs * logRs * logRs );
  kelvinSeco = R_ths - ((Vs*Vs)/(K * Rseco));
  TSeco = (kelvinSeco-273.15)-0.10;    // se puede corregir el offset restando o sumando la variable kelvin 
  }

//------------------------------bulbo húmedo
  float TemperaturaHumedo ()
                                 {                                     
totalBH = totalBH - readingsBH[readIndexBH];
  readingsBH[readIndexBH] = analogRead(InputHumedo);
  totalBH = totalBH + readingsBH[readIndexBH];
  readIndexBH = readIndexBH + 1;
  
  if (readIndexBH >= numReadingsBH)
    readIndexBH = 0;
    
  averageBH = totalBH / numReadingsBH;
  
 // Calculo Temperatura Bulbo Humedo-Promedio 
  
  float VRh= averageBH; 
  Vh =( VRh )*( Vcc/1023.0);   
  Rhumedo = (Rch * Vh ) / (Vcc - Vh);
  logRh=log(Rhumedo);
  R_thh = 1.0 / (Ah + Bh * logRh + Ch * logRh * logRh * logRh );
  kelvinHumedo = R_thh - ((Vh*Vh)/(K * Rhumedo));
  THumedo = (kelvinHumedo-273.15) - 0.14 ;    // se puede corregir el offset restando o sumando la variable kelvin 
}
